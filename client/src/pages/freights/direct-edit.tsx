...
// Componente para edição direta sem formulário React Hook Form
export default function DirectFreightEdit() {
  ...
  useEffect(() => {
    async function loadFreight() {
      ...
      try {
        ...
        const response = await fetch(`/api/freights/${freightId}`);
        if (!response.ok) {
  ...
  // Função para salvar as alterações diretamente usando fetch
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/freights/${freightId}/update-value`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ freightValue: valorNumerico }),
        credentials: 'include'
      });
  ...
  return (
    <div className="container py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Edição Direta do Valor</h1>
      ...
      <Card className="max-w-md mx-auto">
        <CardHeader>
          <CardTitle>Atualizar Valor do Frete</CardTitle>
          <CardDescription>
            Edição direta sem processamento complexo
          </CardDescription>
        </CardHeader>
