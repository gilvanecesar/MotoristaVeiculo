...
- **Campaign Management System**: Admin interface (`/admin/campaigns`) to manage promotional messages displayed in WhatsApp sharing. Features include: campaign creation/editing with randomization toggle, active/inactive status control, multiple message variations per campaign, message-level activation, and automatic integration with freight sharing. Campaign messages are dynamically fetched from the database and randomly selected when sharing freights via WhatsApp. Database tables: `campaigns` (stores campaign metadata) and `campaign_messages` (stores message variations).
