...
async function calculateDistanceBetweenCities(originCity: string, destinationCity: string): Promise<number> {
  ...
  // Rota para gerar e baixar PDF do resumo executivo
  app.get("/api/download/executive-summary", async (req: Request, res: Response) => {
    try {
      const fs = require('fs');
      const path = require('path');
      ...
      const mdFilePath = path.join(process.cwd(), 'RESUMO_EXECUTIVO_INVESTIDORES.md');
      
      if (!fs.existsSync(mdFilePath)) {
        return res.status(404).json({ error: "Arquivo de resumo executivo não encontrado" });
      }
      ...
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', 'attachment; filename="QUERO_FRETES_Resumo_Executivo.pdf"');
      
      // Pipe do PDF para response
      doc.pipe(res);
      
      // Converter markdown para PDF de forma simples
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('# ')) {
          ...
          doc.text('• ' + line.replace(/^[-*] /, ''), 70, y);
          y += 15;
        } else if (line.startsWith('**') && line.endsWith('**')) {
          // Texto em negrito
          doc.fontSize(12).font('Helvetica-Bold');
          doc.text(line.replace(/\*\*/g, ''), 50, y);
          y += 18;
        } else if (line.trim()) {
          // Texto normal
          doc.fontSize(11).font('Helvetica');
          const text = line.replace(/\*\*(.*?)\*\*/g, '$1'); // Remove markdown bold
          doc.text(text, 50, y);
          y += 15;
        } else {
          // Linha vazia
      ...
      console.error("Erro ao gerar PDF do resumo executivo:", error);
      res.status(500).json({ error: "Erro interno do servidor" });
    }
  });
